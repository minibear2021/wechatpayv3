...
5. 支持微信支付直连模式和服务商模式，截至2021年11月04日，已适配[直连模式](https://pay.weixin.qq.com/wiki/doc/apiv3/apis/index.shtml)中所有v3版接口，[服务商模式](https://pay.weixin.qq.com/wiki/doc/apiv3_partner/apis/index.shtml)中所有v3接口(“电商收付通”除外)。
...
| 经营能力 | 微信支付分（公共API） | 创建支付分订单 | payscore_create | 是 | 否 |
| 经营能力 | 微信支付分（公共API） | 查询支付分订单 | payscore_query | 是 | 否 |
| 经营能力 | 微信支付分（公共API） | 取消支付分订单 | payscore_cancel | 是 | 否 |
...
初始化时候指定partner_mode=True即可，需要注意的是，一部分接口为直连商户专有，一部分接口为服务商模式专有，另有部分接口同时兼容直连商户和服务商，这些接口在两种模式下个别参数要求会稍有不同。
