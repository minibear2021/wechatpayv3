...
async def payscore_direct_complete(self, out_order_no, openid, service_id, service_introduction, post_payments,
...
async def payscore_permission_terminate(self, service_id, reason, authorization_code=None, openid=None, appid=None):
...
async def payscore_create(self, out_order_no, service_id, service_introduction, time_range,
...
async def payscore_modify(self, out_order_no, service_id, post_payments, total_amount, reason, post_discounts=None, appid=None):
    """修改订单金额
    :param out_order_no: 商户服务订单号，商户系统内部服务订单号（不是交易单号），要求此参数只能由数字、大小写字母_-|*组成，且在同一个商户号下唯一。示例值:'1234323JKHDFE1243252'
    :param service_id: 服务ID，该服务ID有本接口对应产品的权限。示例值:'500001'
...
async def payscore_complete(self, out_order_no, service_id, post_payments, total_amount, post_discounts=None,
...
async def payscore_sync(self, out_order_no, service_id, scene_type='Order_Paid', detail={'paid_time': None}, appid=None):
    """同步服务订单信息
    :param out_order_no: 商户服务订单号，商户系统内部服务订单号（不是交易单号），要求此参数只能由数字、大小写字母_-|*组成，且在同一个商户号下唯一。示例值:'1234323JKHDFE1243252'
    :param service_id: 服务ID，该服务ID有本接口对应产品的权限。示例值:'500001'
...
async def payscore_refund(self, transaction_id, out_refund_no, amount, reason=None,
...
async def payscore_merchant_bill(self, bill_date, service_id, tar_type='GZIP', encryption_algorithm='AEAD_AES_256_GCM'):
    """商户申请获取对账单
